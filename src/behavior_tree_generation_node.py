...
class Pipeline:
    def __init__(self, generate_file, prompt_dir,
                 sub_tree_dir, api_key, model_name):
        ...
        self.task_label = tk.Label(
            self.upper_frame,
            text="Enter target task description:")
        ...
        self.ed_label = tk.Label(self.middle_frame,
                                 text="Prompts about environment descriptions: ")
        ...
        self.ed_selector = tk.OptionMenu(
            self.middle_frame, self.ed_variable, *self.options)
        self.ed_selector.config(
            height=2, width=15, font=(
                'Helvatical bold', 20))

        self.rs_label = tk.Label(self.middle_frame,
                                 text="Prompts about robot sensing abilities: ")
        ...
        self.rs_selector = tk.OptionMenu(
            self.middle_frame, self.rs_variable, *self.options)
        self.rs_selector.config(
            height=2, width=15, font=(
                'Helvatical bold', 20))

        self.sb_label = tk.Label(self.middle_frame,
                                 text="Prompts about source behavior trees: ")
        ...
        self.sb_selector = tk.OptionMenu(
            self.middle_frame, self.sb_variable, *self.options)
        self.sb_selector.config(
            height=2, width=15, font=(
                'Helvatical bold', 20))

        self.es_label = tk.Label(
            self.middle_frame,
            text="Prompts about existing sub trees: ")
        ...
        self.es_selector = tk.OptionMenu(
            self.middle_frame, self.es_variable, *self.options)
        self.es_selector.config(
            height=2, width=15, font=(
                'Helvatical bold', 20))

        self.ed_label_2 = tk.Label(
            self.middle_frame,
            text="Current prompt about environment descriptions:")
        self.ed_label_2.config(font=('Helvatical bold', 20))

        self.rs_label_2 = tk.Label(
            self.middle_frame,
            text="Current prompt about robot sensing abilities:")
        self.rs_label_2.config(font=('Helvatical bold', 20))

        self.sb_label_2 = tk.Label(
            self.middle_frame,
            text="Current prompt about source behavior trees:")
        self.sb_label_2.config(font=('Helvatical bold', 20))

        self.es_label_2 = tk.Label(
            self.middle_frame,
            text="Current prompt about existing sub trees:")
        self.es_label_2.config(font=('Helvatical bold', 20))

        self.task_button = tk.Button(
            self.lower_frame,
            text="Submit",
            command=self.task_submit)
        self.task_button.config(
            height=2, width=15, font=(
                'Helvatical bold', 20))

        self.view_prompt_ed_button = tk.Button(
            self.middle_frame,
            text="View detail and edit",
            command=self.view_prompt_ed)
        self.view_prompt_ed_button.config(
            height=2, width=15, font=(
                'Helvatical bold', 20))
        self.view_prompt_rs_button = tk.Button(
            self.middle_frame,
            text="View detail and edit",
            command=self.view_prompt_rs)
        self.view_prompt_rs_button.config(
            height=2, width=15, font=(
                'Helvatical bold', 20))
        self.view_prompt_sb_button = tk.Button(
            self.middle_frame,
            text="View detail and edit",
            command=self.view_prompt_sb)
        self.view_prompt_sb_button.config(
            height=2, width=15, font=(
                'Helvatical bold', 20))
        self.view_prompt_es_button = tk.Button(
            self.middle_frame,
            text="View detail and edit",
            command=self.view_prompt_es)
        self.view_prompt_es_button.config(
            height=2, width=15, font=(
                'Helvatical bold', 20))
    ...
    def view_prompt_ed(self):
        def on_popup_window_close():
            self.input_environment_descriptions = text_label.get(
                "1.0", tk.END).strip() + "\n"
    ...
    def view_prompt_es(self):
        ...
        text_label.insert(
            ...
            self.get_single_prompt(
                self.prompt_dir,
                "existing_sub_trees.txt"))

    def view_prompt_rs(self):
        def on_popup_window_close():
            self.input_robot_sensing = text_label.get(
                "1.0", tk.END).strip() + "\n"
            popup_window.destroy()
    ...
    def view_prompt_sb(self):
        ...
        text_label.insert(
            ...
            self.get_single_prompt(
                self.prompt_dir,
                "source_behavior_tree.txt"))

    def task_submit(self):
        self.status_environment_descriptions = self.input_process(
            self.ed_variable.get())
        ...
        self.status_existing_subtree = self.input_process(
            self.es_variable.get())
