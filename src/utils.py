...
def extract_phase(layer, text):
    pattern = re.compile(r'Do phase (' + r'\d+-' * (layer - 1) + r'\d+)'  # captures index according to the specified layer depth
                         # captures conditionals like "if 1 success" or "if 1-1
                         # failed"
                         r'(?: if (' + r'\d+-' * (layer - 1) + \
                         r'\d+)? (success|failed))?'
...
def classify_group(input_struct):
    ...
    for item in input_struct:
        ...
        # Use all parts except the last one as the key
...
def update_content(subtree_in_same_root, command_list):
    for subtree in subtree_in_same_root:
        for potential_node in subtree:
            if detect_string('Do', potential_node['content']) or detect_string(
                    'phase', potential_node['content']) or detect_string('Procedure', potential_node['content']):
                potential_node['minimized_subtree'] = False
            else:
                potential_node['minimized_subtree'] = True
                potential_node['content'] = check_and_cut(
                    potential_node['content'], command_list)
...
def append_subtree(tree_nodes, id, existing_string, tab_size):
    ...
    if len(tree_nodes[id].get_childs(tree_nodes[id])) == 0:
        ...
        for tree in tree_nodes[id].get_childs(tree_nodes[id]):
            existing_string = append_subtree(
                tree_nodes, tree, existing_string, tab_size + 1)
        return existing_string
...
def parsing(input_text):
    ...
    while True:
        _, subtree_in_same_layer = extract_phase(
            nesting_layer_num, procedure_text)
        subtree_in_same_root = classify_group(subtree_in_same_layer)
        subtree_in_same_root = update_content(
            subtree_in_same_root, command_list)
...
def create_tree(root_list, tree_list):
    ...
    for root in root_list:
        ...
        if len(logic_list) == 1 and fallback_list is None and sequence_list is None:
            nodes[parent_register].set_type('<sequence>')
            # print('single as sequence')

        elif fallback_list is None:
            nodes[parent_register].set_type('<sequence>')
            # print('single sequence')

        elif sequence_list is None:
            nodes[parent_register].set_type('<fallback>')
            # print('single fallback')

        elif fallback_list is not None and sequence_list is not None and len(fallback_list) == 1 and len(sequence_list) == 1:
            nodes[parent_register].set_type('<sequence>')

            children_list = nodes[parent_register].get_childs(
                nodes[parent_register])
            ...
            nodes[str(dynamic_index)].children.append(
                nodes[children_list[start_bit]])
            nodes[str(dynamic_index)].children.append(
                nodes[children_list[end_bit]])
            ...
            nodes[parent_register].children.remove(
                nodes[children_list[start_bit]])
            nodes[parent_register].children.remove(
                nodes[children_list[end_bit]])
    ...
    for line in lines:
        if line.startswith("\t"):
            ...
            if stripped_line.startswith("\"") and stripped_line.endswith("\""):
                ...
                extracted_string = stripped_line[1:-1]
